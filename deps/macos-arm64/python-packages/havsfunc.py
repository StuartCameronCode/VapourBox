...
def QTGMC(Input, Preset='Slower', TR0=None, TR1=None, TR2=None, Rep0=None, Rep1=0, Rep2=None, EdiMode=None, RepChroma=True, NNSize=None, NNeurons=None, EdiQual=1, EdiMaxD=None, ChromaEdi='',
    ...
    if maxTR > 0:
        ...
        bVec1 = core.mv.Analyse(srchSuper, isb=True, delta=1, **_fix_mv_args(analyse_args))
        fVec1 = core.mv.Analyse(srchSuper, isb=False, delta=1, **_fix_mv_args(analyse_args))
    if maxTR > 1:
        bVec2 = core.mv.Analyse(srchSuper, isb=True, delta=2, **_fix_mv_args(analyse_args))
        fVec2 = core.mv.Analyse(srchSuper, isb=False, delta=2, **_fix_mv_args(analyse_args))
    if maxTR > 2:
        bVec3 = core.mv.Analyse(srchSuper, isb=True, delta=3, **_fix_mv_args(analyse_args))
        fVec3 = core.mv.Analyse(srchSuper, isb=False, delta=3, **_fix_mv_args(analyse_args))
    ...
    if ShutterBlur > 1:
        recalculate_args = dict(thsad=ThSAD1, blksize=rBlockSize, overlap=rOverlap, search=Search, searchparam=SearchParam, truemotion=TrueMotion, _lambda=rLambda, pnew=PNew, dct=DCT,
                                chroma=ChromaMotion)
        sbBVec1 = core.mv.Recalculate(srchSuper, bVec1, **_fix_mv_args(recalculate_args))
        sbFVec1 = core.mv.Recalculate(srchSuper, fVec1, **_fix_mv_args(recalculate_args))
    elif ShutterBlur > 0:
        sbBVec1 = bVec1
        sbFVec1 = fVec1
...
def LUTDeCrawl(input, ythresh=10, cthresh=10, maxdiff=50, scnchg=25, usemaxdiff=True, mask=False):
    if not isinstance(input, vs.VideoNode) or input.format.color_family != vs.YUV or input.format.bits_per_sample > 10:
        raise TypeError('LUTDeCrawl: This is not an 8-10 bit YUV clip')
...
def MCTemporalDenoise(i, radius=None, pfMode=3, sigma=None, twopass=None, useTTmpSm=False, limit=None, limit2=None, post=0, chroma=None, refine=False, deblock=False, useQED=None, quant1=None,
    ...
    if p is not None:
        ...
        p = core.dfttest.DFTTest(i, tbsize=1, sigma=10.0, planes=planes)
    else:
    ...
    f1v = core.mv.Analyse(pMVS, isb=False, delta=1, **_fix_mv_args(analyse_args))
    b1v = core.mv.Analyse(pMVS, isb=True, delta=1, **_fix_mv_args(analyse_args))
    if refine:
        f1v = core.mv.Recalculate(rMVS, f1v, **_fix_mv_args(recalculate_args))
        b1v = core.mv.Recalculate(rMVS, b1v, **_fix_mv_args(recalculate_args))
    if radius > 1:
        f2v = core.mv.Analyse(pMVS, isb=False, delta=2, **_fix_mv_args(analyse_args))
        b2v = core.mv.Analyse(pMVS, isb=True, delta=2, **_fix_mv_args(analyse_args))
        if refine:
            f2v = core.mv.Recalculate(rMVS, f2v, **_fix_mv_args(recalculate_args))
            b2v = core.mv.Recalculate(rMVS, b2v, **_fix_mv_args(recalculate_args))
    if radius > 2:
        f3v = core.mv.Analyse(pMVS, isb=False, delta=3, **_fix_mv_args(analyse_args))
        b3v = core.mv.Analyse(pMVS, isb=True, delta=3, **_fix_mv_args(analyse_args))
        if refine:
            f3v = core.mv.Recalculate(rMVS, f3v, **_fix_mv_args(recalculate_args))
            b3v = core.mv.Recalculate(rMVS, b3v, **_fix_mv_args(recalculate_args))
    if radius > 3:
        f4v = core.mv.Analyse(pMVS, isb=False, delta=4, **_fix_mv_args(analyse_args))
        b4v = core.mv.Analyse(pMVS, isb=True, delta=4, **_fix_mv_args(analyse_args))
        if refine:
            f4v = core.mv.Recalculate(rMVS, f4v, **_fix_mv_args(recalculate_args))
            b4v = core.mv.Recalculate(rMVS, b4v, **_fix_mv_args(recalculate_args))
    if radius > 4:
        f5v = core.mv.Analyse(pMVS, isb=False, delta=5, **_fix_mv_args(analyse_args))
        b5v = core.mv.Analyse(pMVS, isb=True, delta=5, **_fix_mv_args(analyse_args))
        if refine:
            f5v = core.mv.Recalculate(rMVS, f5v, **_fix_mv_args(recalculate_args))
            b5v = core.mv.Recalculate(rMVS, b5v, **_fix_mv_args(recalculate_args))
    if radius > 5:
        f6v = core.mv.Analyse(pMVS, isb=False, delta=6, **_fix_mv_args(analyse_args))
        b6v = core.mv.Analyse(pMVS, isb=True, delta=6, **_fix_mv_args(analyse_args))
        if refine:
            f6v = core.mv.Recalculate(rMVS, f6v, **_fix_mv_args(recalculate_args))
            b6v = core.mv.Recalculate(rMVS, b6v, **_fix_mv_args(recalculate_args))
...
def SMDegrain(input, tr=2, thSAD=300, thSADC=None, RefineMotion=False, contrasharp=None, CClip=None, interlaced=False, tff=None, plane=4, Globals=0,
    ...
    if not GlobalR:
        if preclip:
            ...
            pref = core.std.MaskedMerge(core.dfttest.DFTTest(inputP, tbsize=1, sigma=10.0, planes=planes),
                                        inputP,
    ...
    if not GlobalR:
        ...
        if interlaced:
            if tr > 2:
                bv6 = core.mv.Analyse(super_search, isb=True, delta=6, **_fix_mv_args(analyse_args))
                fv6 = core.mv.Analyse(super_search, isb=False, delta=6, **_fix_mv_args(analyse_args))
                if RefineMotion:
                    bv6 = core.mv.Recalculate(Recalculate, bv6, **_fix_mv_args(recalculate_args))
                    fv6 = core.mv.Recalculate(Recalculate, fv6, **_fix_mv_args(recalculate_args))
            if tr > 1:
                bv4 = core.mv.Analyse(super_search, isb=True, delta=4, **_fix_mv_args(analyse_args))
                fv4 = core.mv.Analyse(super_search, isb=False, delta=4, **_fix_mv_args(analyse_args))
                if RefineMotion:
                    bv4 = core.mv.Recalculate(Recalculate, bv4, **_fix_mv_args(recalculate_args))
                    fv4 = core.mv.Recalculate(Recalculate, fv4, **_fix_mv_args(recalculate_args))
        else:
            if tr > 2:
                bv3 = core.mv.Analyse(super_search, isb=True, delta=3, **_fix_mv_args(analyse_args))
                fv3 = core.mv.Analyse(super_search, isb=False, delta=3, **_fix_mv_args(analyse_args))
                if RefineMotion:
                    bv3 = core.mv.Recalculate(Recalculate, bv3, **_fix_mv_args(recalculate_args))
                    fv3 = core.mv.Recalculate(Recalculate, fv3, **_fix_mv_args(recalculate_args))
            bv1 = core.mv.Analyse(super_search, isb=True, delta=1, **_fix_mv_args(analyse_args))
            fv1 = core.mv.Analyse(super_search, isb=False, delta=1, **_fix_mv_args(analyse_args))
            if RefineMotion:
                bv1 = core.mv.Recalculate(Recalculate, bv1, **_fix_mv_args(recalculate_args))
                fv1 = core.mv.Recalculate(Recalculate, fv1, **_fix_mv_args(recalculate_args))
        if interlaced or tr > 1:
            bv2 = core.mv.Analyse(super_search, isb=True, delta=2, **_fix_mv_args(analyse_args))
            fv2 = core.mv.Analyse(super_search, isb=False, delta=2, **_fix_mv_args(analyse_args))
            if RefineMotion:
                bv2 = core.mv.Recalculate(Recalculate, bv2, **_fix_mv_args(recalculate_args))
                fv2 = core.mv.Recalculate(Recalculate, fv2, **_fix_mv_args(recalculate_args))
    else:
