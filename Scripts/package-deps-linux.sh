...
package_arch() {
    ...
    # The CPU tier (issue #92) is whatever download-deps-linux.sh stamped into
    ...
    local PLATFORM_KEY="linux-$ARCH_NAME"
    [ "$TIER" = "v2" ] && PLATFORM_KEY="linux-$ARCH_NAME-v2"
...
PY
    ...
    if [ -n "$MISSING" ]; then
        ...
        echo "A plugin build/download likely failed - check the download-deps-linux.sh output." >&2
